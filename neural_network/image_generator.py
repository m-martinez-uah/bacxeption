...
class DataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, id_list, id_dict, dim, batch_size=32,
                 n_classes=2, train=True, shuffle=True):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            current_img = imread(os.path.join('bacxeption/data', str(self.id_dict[ID]),
                                              list_IDs_temp[i]),
                                 # as_gray=True)
                                 as_gray=False)
